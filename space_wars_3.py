...
classifier = ml.load_model('./model/ExtremelyFastDecisionTreeClassifier_model.pkl')
...
def prediction_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count,streamkmeans,classifier):
	global A0, A1
	...
	# a11 = a3/a9
	# a12 = a2/a10
	#X = [a0, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, timestamp]
	#X = {'a0':a0,'a1':a1,'a2':a2,'a3':a3,'a4':a4,'a5':a5,'a6':a6,'a7':a7,'a8':a8,'a9':a9,'a10':a10}
	X = {'a2':a2,'a3':a3,'a4':a4,'a5':a5,'a6':a6,'a7':a7,'a8':a8,'a9':a9,'a10':a10}
	#X = dict(zip(range(11), X))
	timestamp = time.time()
	if timestamp - start_time >= 15 and destroyed_enemy_count > 5:
		y = ml.predict_gamer(X, classifier)
	else:
		y = None
	return y,streamkmeans,classifier
...
#	global db_connection
	...
	while high_scores_screen:
		...
		top_5 = high_scores.top_5()

		index = 0
		for entry in top_5:
			# timestamp, name, score, date
			index += 1
			message_display_left(str(entry[0]), font_medium, yellow, int((screen_sizeX * 1/4)), int(screen_sizeY *(2+index)/10))
			message_display_right(str(entry[1]), font_medium, yellow, int((screen_sizeX * 3/4)), int(screen_sizeY *(2+index)/10))
...
def menu():
	...
	while intro_screen:
		...
		screen.blit(textinput.surface, (int(screen_sizeX/2)-50, int((screen_sizeY/3)+70)))
		...
		if yes_button.clicked and username != '':
			intro_screen = False
			quit_game = False
			#print('username=', username)
		elif yes_button.clicked and username == '':
			message_display_center('Type your name!', font_medium, red, int(screen_sizeX/2), int((screen_sizeY/3)-70))
			quit_game = False
...
def show_game_over(screen_sizeX, screen_sizeY, score, high_score, coin_count, user_type='NONE'):
...
def save_collection_data(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count):
	...
	if sum([A0, A1, A2, A3, A4, A5]) != 0:
		timestamp = time.time()
		data = {'a0':A0,'a1':A1,'a2':A2,'a3':A3,'a4':A4,'a5':A5,'a6':A6,'a7':A7,'a8':A8,'a9':A9,'a10':A10,'timestamp':timestamp}
		high_scores.save_train_data(data,username)
...
while not quit_game:
	...
	pygame.draw.rect(window, black, (100, 100, 200, 30), 2)

    # Render the text
	font = pygame.font.SysFont(None, 24)
	text_surface = font.render(input_text, True, black)
	window.blit(text_surface, (105, 105))
...
#			score += level_score_increase
		...
		if game_over:
			...
			thread.cancel()
			user_type,streamkmeans,classifier = prediction_user_type(level, keyX_pressed_count, keyY_pressed_count, respawn_enemy_count, respawn_coin_count,streamkmeans,classifier)
			if score > 0:
				high_scores.publish_online_score(PLAYER_NAME,score)
				
			
			val = high_scores.top_5()
			session_high_score = max(val, key=lambda x: x[1])
			show_game_over(screen_sizeX, screen_sizeY, score, session_high_score[1], coin_count,user_type=user_type)
			show_score(score, level, PLAYER_NAME, coin_count, game_over=True, user_type=user_type)
			show_online_score()
			...
			for i in range(num_of_enemies):
				...
				if enemy[i].explosion_counter >= 1:
					...
					if is_collision(enemy[i], player):
						...
						try:
							session_high_score = session_high_score[1]
						except:
							pass
						...
						streamkmeans,classifier = ml.train_model(streamkmeans,classifier,start_time)
